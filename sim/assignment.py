...
class World(object):
    ...
    def __init__(self, sensor_angles=(0,), luminance=1.0, light_coords=(10.0, 0.0, -0.1), v_max=1.0, agent_radius=0.5,
                 sensor_noise=0.01, motor_noise=0.5, dt=0.1, seed=None):
    ...
    def animate(self, poses, sensations, speedup=5):
        ...
        sensors = [patches.Ellipse(xy=(r * np.cos(theta), r * np.sin(theta)), width=0.2, height=0.2, fc='b') for theta
                   in self.sensors]
        ...
        ani = FuncAnimation(fig, anim, init_func=init, frames=None, interval=1000 * self.dt / speedup, blit=True,
                            save_count=len(poses))
